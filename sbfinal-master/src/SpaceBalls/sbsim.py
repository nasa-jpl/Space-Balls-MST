...
class SpaceBallsSim():
    ...
    def __init__(self, input_name='sb_inputs', verbose=True):

        self.input_name = input_name
        #default_settings = 'input_files.' + self.input_name
        #self.userfile = importlib.import_module(default_settings)
        #importlib.reload(self.userfile)
        ...
        self.out_dir_csv = '/media/monte_share/output_files/' + self.input_name + '/data_output/'   # TODO: flexible output directory?
        ...
        self.sampletime = self.userfile.OUTPUT_SETTINGS['output_sample_time']
    ...
    def run_daily_loop(self):
        ...
        self.spacecraft._set_initial_state_Monte(self.daily_th_array[days].start_epoch) 
    ...
    def _set_boa(self): #boa):
        ...
        boa_name = rad_settings.radiation_settings_from_EEI_truth_name(
            self.userfile.FORCE_SETTINGS["EEI_truth"])["ephemerides"]
    ...
    def _set_time_handler(self):
        ...
        self.start_Time = Time(self.userfile.TIMESTART)
        self.finish_Time = Time(self.userfile.TIMEFINISH)
    ...
    def _create_spacecraft_set(self):
        ...
        for i in range(n):
            name = self.userfile.SC_INPUT['name'] + '_' + str(i)
            sc_i = Spacecraft(name, self)
    ...
    def _set_propagation(self):
        ...
        integState.addState(self.daily_th.start_epoch, self.spacecraft.name, "Earth", "EME2000", self.spacecraft.initState) # ie frameName = "Earth Inertial at Launch"
        integState.addMass(self.daily_th.start_epoch, self.spacecraft.name, self.spacecraft.mass)
...
class Spacecraft():
    ...
    #            mass=100, cd=2.2, diffReflect=0.0, diffDegrade=1.0, boa=None, kep_0=None): # , state_0=[] 
        ...
        self.name = name
        self.shape_name = sim_instance.userfile.SC_INPUT['shape']
        area = sim_instance.userfile.SC_INPUT['area']                             # [m^2]
        self.area = area * units.m * units.m                                      # [m^2]
        self.equivalent_radius = np.sqrt( area /np.pi ) * units.m             # [m]
        self.mass = sim_instance.userfile.SC_INPUT['mass'] * units.kg             # [kg]
        self.cd = sim_instance.userfile.SC_INPUT['cd']                            # []
        self.diffReflect = sim_instance.userfile.SC_INPUT['diffReflect']          # [] 
        self.diffDegrade = sim_instance.userfile.SC_INPUT['diffDegrade']          # []
        self.specReflect = sim_instance.userfile.SC_INPUT['specReflect']          # [] 
        self.specDegrade = sim_instance.userfile.SC_INPUT['specDegrade']          # []

        self.boa = sim_instance.boa
        self.start_epoch = sim_instance.th.start_epoch
        self.png_out_folder = sim_instance.out_dir_png
        
        self.existing_days = sim_instance.existing_days
        
        if self.existing_days==0:
            self.kep_0 =  sim_instance.userfile.SC_KEP_0
        else:
