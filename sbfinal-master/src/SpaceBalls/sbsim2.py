...
from SpaceBalls.utils import load_input_file, progress_bar # , get_mesh_from_fibonacci_sphere
...
class SpaceBallsSim():
    ...
    def __init__(self, input_names=['sb_inputs'], verbose=True):

        self.input_names = [None] * len(input_names)
        self.userfiles = [None] * len(input_names)

        for i, input_name in enumerate(input_names):
            self.input_names[i] = input_name
            self.userfiles[i] = load_input_file(input_name)
        ...
        # else:
        n_rings = self.userfiles[0].INTEGRATION_SETTINGS['n_rings_albedo'] # TODO: database
        self.n_rings_albedo = n_rings
        
        #self.wgs84_albedo = self.userfile.FORCE_SETTINGS['WGS84_albedo_shape']
        #self.out_dir_csv = './output_files/' + self.input_name + '/data_output/'
        #self.out_dir_png = './output_files/' + self.input_name + '/img_output/'
        
        self.out_dir_csv = ['/media/monte_share/output_files/' + input_name + '/data_output/' for input_name in self.input_names]  # TODO: flexible output directory?
        self.out_dir_png = ['/media/monte_share/output_files/' + input_name + '/img_output/'  for input_name in self.input_names]
        
        # TODO (just commented): existing days in interrupted propagation
        ...
        Shape._set_earth_shape(self.boa)
        ...
        # self.spacecraft = self.sc_array[0] ## temporary patch - only 1 spacecraft

        self.NadirFrames = [Frames.set_NadirFrame(self.boa, self.th, sc.name) for sc in self.sc_array]
        ...
        self.sampletime = self.userfiles[0].OUTPUT_SETTINGS['output_sample_time']
    ...
    def run_daily_loop(self):
        ...
        for i in range(len(self.sc_array)):
            self.sc_array[i]._set_initial_state_Monte(self.daily_th_array[days].start_epoch) 
        ...
        while days <= (self.loop_ndays-1):
            ...
            self.run_integrator()
            t2 = time.time()
            print(f"Time spent for propagation: {t2-t1}")
            self.write_output(daily_out_subdir)
    ...
    def _set_boa(self): #boa):
        ...
        boa_name = rad_settings.radiation_settings_from_EEI_truth_name(
            self.userfiles[0].FORCE_SETTINGS["EEI_truth"])["ephemerides"] # TODO: database
    ...
    def _set_time_handler(self):
        ...
        self.start_Time = Time(self.userfiles[0].TIMESTART) # TODO: database (timespan is embedded in EEI truth)
        self.finish_Time = Time(self.userfiles[0].TIMEFINISH)
    ...
    def _create_spacecraft_set(self):
        ...
        n = len(self.userfiles)
        self.sc_array = [None]*n

        for i, userfile in enumerate(self.userfiles):
            name = self.input_names[i] # userfile.SC_INPUT['name'] + '_' + str(i)
            sc_i = Spacecraft(name, userfile.SC_KEP_0, userfile.SC_INPUT, self.th.start_epoch, self.boa)
    ...
    def run_integrator(self):
        ...
        self._set_propagation()
        self._run_propagation()


    def _set_propagation(self):

        # myforce & low thrust force ???
        #self.query = M.TrajQuery(self.boa, self.spacecraft.name, 'Earth', 'IAU Earth Pole')
        self.query_array = [] 
        for sc in self.sc_array:
            self.query_array.append(M.TrajQuery(self.boa, sc.name, 'Earth', 'IAU Earth Pole'))
        
        integState = M.IntegrationState(self.boa)
        for sc in self.sc_array:
            integState.addState(self.daily_th.start_epoch, sc.name, "Earth", "EME2000", sc.initState) # ie frameName = "Earth Inertial at Launch"
            integState.addMass(self.daily_th.start_epoch, sc.name, sc.mass)
        
        self.prop = M.DivaPropagator(self.boa, "DIVA", integState)
        self.prop.setMinStep(1e-9*units.sec)
        self.prop.setMaxStep(1e9*units.sec)

        for sc_i, f_array in enumerate(self.force_manager.forces):
            for f in f_array:
                self.prop.addForce(f(self.boa, self.sc_array[sc_i].name))

        for sc in self.sc_array:
            M.DivaTraj(self.boa, sc.name, self.prop)
            M.DivaMass(self.boa, sc.name, self.prop)
        ...
        StopEvents = [ M.CoordinateEvent(query, M.Conic.semiMajorAxis(),  constants.earth_radius(monte_units=True) + 100*units.km , 
                                         M.CoordinateEvent.DECREASING ) for query in self.query_array]
    ...
    def _run_propagation(self):
        ...
        self.prop.create( self.boa2, self.daily_th.start_epoch, self.daily_th.stop_epoch )  # main propagation method. 
        ...
        trajLifetime = (endTime - self.daily_th.start_epoch).convert('days')
    ...
    def write_output(self, daily_out_subdir):
        
        data_out_dir = self.out_dir_csv + daily_out_subdir
        ...
        output_manager = OutputManager(self)
...
class TimeHandler():
    ...
    #def __init__(self, start_Time, duration=1.0 ):  #duration is in days
        ...
        self.stop_epoch = self.start_epoch + duration * 86400*units.sec
...
class OutputManager():
    ...
    def __init__(self, sim_instance):
        ...
        self.th = sim_instance.daily_th  # TODO: not call it "daily" - why not?
...
class ForceManager():
    ...
    def __init__(self, sim_instance):
        self.boa = sim_instance.boa
        self.settings_dict = sim_instance.userfiles[0].FORCE_SETTINGS # TODO: database
        self.rad_settings_dict = rad_settings.radiation_settings_from_EEI_truth_name(
            self.settings_dict["EEI_truth"]) # this ensures that all radiation force settings are consistent with the set EEI
        self.verbose = sim_instance.verbose
        #self.sc = sim_instance.spacecraft
        self.sc_array = sim_instance.sc_array
        self.th = sim_instance.daily_th   # TODO
        self.n_rings_albedo = sim_instance.n_rings_albedo
        #self.wgs84_albedo = sim_instance.wgs84_albedo 

        self.forces = [[] for _ in self.sc_array]
        self.pressforces = [[] for _ in self.sc_array]

    def set_forces(self):

        if self.verbose: print('\n\nsettings: ', self.settings_dict, '\n\n')
        self.force_names = [[] for _ in self.sc_array]

        for i, sc in enumerate(self.sc_array):
            self._load_gravity(sc.name, i)
            self._load_drag(sc, i)
            self._load_srp(sc, i)
            self._load_albedo(sc, i)
    ...
    def _load_gravity(self, scname, sc_i):

        
        def get_Earth_SH_gravity_model():
            if self.settings_dict['Earth_grav']=="GGM02C":
                from config.earth.gravity import GGM02c_monte_grv as GGM02C
                return GGM02C
            else:
                raise ValueError(f"Unrecognized gravity field {self.settings_dict['Earth_grav']}")

        self.forces[sc_i].append(M.GravityForce)
        grav = M.Gravity(self.boa, scname)
        
        GravModel = get_Earth_SH_gravity_model()

        # from docs: "NOTE: This constructor is obsolete and is kept only for backward compatibility.":
        #EarthHarmonics = M.SphHarmonics(self.boa, "Earth", M.CoordName.IauEarthFixed, GravModel.meanr,  # TODO: is this output useless?? (the line is not)
        #                                GravModel.jCof[:7], 6, GravModel.cCof,GravModel.sCof, GravModel.normalized)
        Nmax = 6 # TODO: database       # TODO: if second input (name) is changed to anything else than Earth, propagation crashes. Why?
        Earth_SH = M.SphHarmonics(self.boa, "Earth", "Earth", M.CoordName.IauEarthFixed, GravModel.meanr,
                                  GravModel.normalized, GravModel.jCof, GravModel.cCof, GravModel.sCof, []) # new constructor TODO: Nmax
        Earth_SH.setDegreeCS(Nmax)
        Earth_SH.setDegreeJ(Nmax)
        
        Earth_SOI = constants.earth_SOI(monte_units=True)
        grav.insert(M.GravityNode("", "", "Earth", Earth_SOI, M.GravityNode.SPHERICAL)) #M.GravityNode.SPHERICAL seems to enable the whole SH field
        #grav.insert(M.GravityNode("", "", "Earth", Earth_SOI))
        
        for body in self.settings_dict['third_body_grav']:
            grav.insert( M.GravityNode("", "", body, 0*units.km))    #, GravityNode.NEWTONIAN) )
            #self.force_names.append(body) # TODO: third bodies (?)

        self.pressforces[sc_i].append(grav)
        self.force_names[sc_i].append("total_grav_"+scname) # TODO: third bodies (?)

        if self.verbose: print("gravity force loaded")
        

    def _load_drag(self, sc, sc_i):
        ...
        self.forces[sc_i].append(M.AtmDragForce)
        ...
        atm = M.AtmDrag(self.boa, sc.name, sc.cd, sc.shape_label, 'DTM') # 'OCO2 Shape', 'DTM')
        self.pressforces[sc_i].append(atm)
        self.force_names[sc_i].append('aero_'+sc.name)
    ...
    def _load_srp(self, sc, sc_i):
        
        self.forces[sc_i].append(M.SolarPressureForce)
        ...
        #create the object:
        solarRadPress = M.SolarPressure(self.boa, sc.name, sc.shape_label, solar_flux)
        ...
        self.pressforces[sc_i].append(solarRadPress)
        self.force_names[sc_i].append('srp_'+sc.name)

        if self.verbose:
            print('srpPress.__class__: ', solarRadPress.__class__ )


    def _load_albedo(self, sc, sc_i):

        self.forces[sc_i].append(M.AlbedoForce)
        ...
        earthAlbedoPress = M.AlbedoPressure( self.boa, sc.name, 
                                                 sc.name+' Shape',  sc.name+' Shape', 
                                                 ["Earth"], threshAngle )
        ...
        self.pressforces[sc_i].append(earthAlbedoPress)
        self.force_names[sc_i].append('erp_'+sc.name)
...
class Spacecraft():
    ...
    def __init__(self, name, sc_kep0_dict, sc_input_dict, start_epoch, boa):   #sim_instance: SpaceBallsSim): # , state_0=[] 
    ...
    #            mass=100, cd=2.2, diffReflect=0.0, diffDegrade=1.0, boa=None, kep_0=None): # , state_0=[] 
        ...
        self.name = name
        self.shape_name = sc_input_dict['shape']
        area = sc_input_dict['area']                             # [m^2]
        self.area = area * units.m * units.m                                      # [m^2]
        self.equivalent_radius = np.sqrt( area /np.pi ) * units.m             # [m]
        self.mass = sc_input_dict['mass'] * units.kg             # [kg]
        self.cd = sc_input_dict['cd']                            # []
        self.diffReflect = sc_input_dict['diffReflect']          # [] 
        self.diffDegrade = sc_input_dict['diffDegrade']          # []
        self.specReflect = sc_input_dict['specReflect']          # [] 
        self.specDegrade = sc_input_dict['specDegrade']          # []

        self.boa = boa
        self.start_epoch = start_epoch
        
        self.existing_days = 0  # sim_instance.existing_days 
        
        if self.existing_days==0:
            self.kep_0 =  sc_kep0_dict
        else:
        ...
        if self.shape_name != 'sphere':
            ...
            self.direct =  Frames.set_NadirECI(self.boa, self.name)
            self.direct2 = Frames.set_SunDirECI(self.boa) # TBC with other config options
    ...
    def _init_shape(self, make_sc_plot=False):
        ...
        if self.shape_name=="sphere":
            ...
            if parts is not None and parts.group(1)=='sphere':
                ...
                self._define_plated_shape(n_faces, load_csv=False)
    ...
    def _define_plated_shape(self, n_faces=128, load_csv=True):
        ...
        if load_csv:
            ...
            # normal_vecs, areas = get_mesh_from_fibonacci_sphere(
        ...
        for i, (vec, area) in enumerate(zip(normal_vecs, areas)):
            ...
            dirbody = M.FixedDir(self.boa, frame_name, vec)  # "(one) typical use is to define a direction that lies along an axis of a coordinate frame"
    ...
    def _set_initial_state_Monte(self, start_epoch=None):
        ...
        if self.existing_days==0:
            self.initState = M.State (
                                M.Conic.semiMajorAxis( self.kep_0['a'] *units.km ),    # 7378.14 mean alt 1000km, 7037.34 is osc, mean is 7028.14; mean alt. 650km  
                                M.Conic.eccentricity( self.kep_0['e'] ),               # 0.0012550 is for frozen orbit
                                M.Conic.inclination( self.kep_0['i'] * units.deg ),    # 99.48 deg for ss orbit at altitude 1000km, 97.99 is inclination for meab alt 650 km    
                                M.Conic.longitudeOfNode( self.kep_0['o'] *units.deg),  # MLTAN 6AM for 90 deg; 12PM for 180 deg   
                                M.Conic.argumentOfPeriapsis( self.kep_0['u'] *units.deg), 
                                M.Conic.trueAnomaly( self.kep_0['theta'] *units.deg),  # just choosing a true anomaly: use (359.9-initArgPeri)        
                                # # EME is inertial equatorial, so this should be correct
                                M.StateInfo (self.boa, start_epoch, self.name, "Earth", "EME2000" ) # frameName = "Earth Inertial at Launch" 
