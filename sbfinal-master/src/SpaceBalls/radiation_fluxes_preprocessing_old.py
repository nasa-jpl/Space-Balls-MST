...
def compute_daily_hist_net_at_altitude(base_data_dir, day_idx, mid_day_jd, rad_config, TSI_1AU_day, grid: Grid, n_cores):
    ...
    t1 = time.time()
    emission_F_day_hist_2 = np.einsum('it,ji,jik,ji,i->jkt', 
                                      toa_L_day_hist, cos_alpha, r_rel, 
                                      1/(r_rel_norm**3), toa_grid.integration_weights,
                                      optimize=True)
    t2 = time.time()
    print(f"Time with einsum: {t2-t1}")
    ...
    emission_F_day_hist = np.stack([res[1] for res in results], axis=2)
    t3 = time.time()
    print(f"Time with multiproc: {t3-t2}")
    print(f"Max diff: {np.max(np.abs(emission_F_day_hist_2 - emission_F_day_hist))}")
    print(f"Max rel. diff: {np.max(np.abs(emission_F_day_hist_2 - emission_F_day_hist)/emission_F_day_hist)}")
...
def compute_fluxes_worker2(i, toa_emission):
    ...
    g = _pool_globals
    emission_F = compute_all_fluxes_at_altitude(
        toa_emission, g['cos_alpha'], g['r_rel_norm'], g['r_rel'], g['weights'], mode="single_step" #method="einsum"
    )
    return i, emission_F
