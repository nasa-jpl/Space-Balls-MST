...
def find_aero_accel(boa, sc, aeroPress, startEpoch, stopEpoch, outframeName, sampletime, userfile): # finds aerodynamic acceleration given an AtmDrag Pressure object 
    ...
    print ('Aerodynamic Acceleration')  

    # Aero Accel Print Section

    pointtimes = M.Epoch.range( startEpoch, stopEpoch, sampletime * units.sec )
    
    header = headers.make_header( userfile )
    print("header: ", header)
    print ('Printing Aerodynamic acceleration every ' +str(sampletime)+ ' secs (m/s**2):')  #section for screen print - can cut out
    ...
    for time in pointtimes:
        ...
        statestring = '%6.2f %3.5f %3.5f %.15f %.15f %.15f %.15f' %( reltime  #section for screen print - can cut out
                    ,longitude
                    ,latitude
                    ,aeroPress.accel(time, 'EME2000')[0]*1000# default accel is in km/s**2
                    ,aeroPress.accel(time, 'EME2000')[1]*1000
                    ,aeroPress.accel(time, 'EME2000')[2]*1000
                    ,aeroPress.accel(time, 'EME2000').mag()*1000
                    ) 
        #print( statestring )

        eme2000ToNadir = coord.rotation(time, outframeName, 'EME2000') #rotation for going from EME2000 to outframeName

        nadirvec = eme2000ToNadir * aeroPress.accel(time, 'EME2000') #transform operation to find vector in nadir frame
...
def aero_accel_csv(outset, outpath, startEpoch, sampletime, userfile):  #csv output

    datestring = convert_Epoch_to_DateStr(startEpoch)
    fileName = outpath + '/csvoutput/' + 'aero_accel_'+datestring+'.csv'
    ...
    headerline1='Printing Aerodynamic acceleration every ' +str(sampletime)+ ' secs (m/s**2):\n\
    Epoch   %s \n' % (startEpoch.format()) + \
    ...
    if userfile.PLOT[ 'ploton' ] &  userfile.PLOT[ 'aero_plot' ]: 
        ...
        df.plot()
        plt.savefig( outpath + '/pngoutput/' + 'aero_accel'+datestring+'.png' )
        plt.show()
        plt.close()
