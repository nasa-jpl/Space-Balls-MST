...
from SpaceBalls.utils import normal_smoother, progress_bar, get_rolling_jd_windows, nanrms, make_dict_hash_key, make_list_str_key
...
for window_name, window_days in zip(smoothing_windows_names, smoothing_windows_days):
    ...
    for sat_names in all_sat_combinations_k3:
        ...
        if (not row_exists) and (not cell_exists):
            ...
            old_file_name = os.path.join(old_folder, 'error_time_series_'+old_name+'.npy')
            if os.path.exists(old_file_name):
                error_time_series = np.load(old_file_name)
            else:
                error_time_series = compute_rolling_avg_error_time_series(SB_constellation, window_days, EEI_time_series_180x360, 
