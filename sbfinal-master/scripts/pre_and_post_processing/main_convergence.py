...
base_sb_tag = 'Ajisai' # 'LAGEOS' # 'GRACE-FO_spherical'
n_days_arc = 30
n_propagations = 20
...
finest_regular_grid = RegularLatLonGrid(0, lmax=2500, quad_type='GLQ')
#finest_regular_grid = QuadratureGrid(0, order=131)

regular_grids = [RegularLatLonGrid(0,  n_lat=int(np.round(180*f)), 
                                        n_lon=int(np.round(360*f))) for f in np.logspace(-1, 0.5, 15)]
lebedev_grids = [QuadratureGrid(0, order=o) for o in lebedev_n_array]
womersley_grids = [QuadratureGrid(0, order=o, default_womersley=True) for o in np.arange(33, 325, 4)]
...
glq_grids = [RegularLatLonGrid(0, lmax=int(l), quad_type='GLQ') for l in l_glq]

N_array = np.round(np.logspace(1.4, 5, 20))
nR_array = np.round(-0.5 + 0.5*np.sqrt((4*N_array - 1)/3))
nR_array = nR_array[:18]
knocke_monte_grids = [KnockeGridMONTE(int(nR)) for nR in nR_array]
